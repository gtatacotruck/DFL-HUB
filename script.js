...
enhancedStyle.textContent = `
  ...
  @keyframes enhancedGlow {
    ...
    50% { 
      filter: drop-shadow(0 0 40px var(--glow-primary)) drop-shadow(0 0 80px var(--glow-secondary));
    }
  }
  
  .enhanced-entrance {
    animation: enhancedSlideUp 0.8s ease forwards;
  }
  
  .logo-glow.enhanced {
    animation: titleGlitch 3s infinite, enhancedGlow 2s infinite;
  }
`;
...
function startEnhancedLoadingSequence() {
  const loader = document.getElementById('loader');
  const site = document.getElementById('site');
  const credits = document.getElementById('credits');
  const logoGlow = document.querySelector('.logo-glow');

  // After 1.5s, hide loader and drop in UI with enhanced animations
  setTimeout(() => {
    loader.classList.add('hidden');

    // Add enhanced class to logo
    logoGlow.classList.add('enhanced');

    // Drop-down stagger for major sections with enhanced timing
    const sections = [
      document.querySelector('.profile-header'),
      document.querySelector('.hero-section'),
      document.querySelector('.content-grid'),
      document.querySelector('.profile-footer')
    ];
    
    sections.forEach((el, i) => {
      if (!el) return;
      el.style.opacity = '0';
      el.style.transform = 'translateY(-150px) rotateX(30deg)';
    ...
    // Enhanced credits tape animation
    setTimeout(() => {
      credits.classList.add('roll');
      credits.style.animation = 'rollAcross 3s ease-out forwards, enhancedGlow 2s infinite';
      
      // Fade credits out after 5s
      setTimeout(() => {
        credits.style.opacity = '0';
        credits.style.transition = 'opacity 0.8s ease';
      }, 5000);
    }, 800);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Kick off enhanced loading sequence
